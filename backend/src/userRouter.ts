...
userRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const body = req.body;
      let whereConditions: any = [isNull(profiles.deleted_at)];
      if (body.searchString?.trim()) {
        whereConditions = [isNull(profiles.deleted_at)];
...
userRouter.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const baseURL =
        appConfig.baseURL?.replace(/\/$/, "") || "http://localhost:3000";
      const imagePath = `${baseURL}/profilePictures/${req.file.filename}`;
      console.log("Saving Image URL:", imagePath);

      const updatedUser = await db
        .update(profiles)
        .set({ profile_image_url: imagePath })
        .where(eq(profiles.guid, req.params.userId))
        .returning();
